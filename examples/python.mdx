...
agent = requests.post(
    'https://suite.sundaypyjamas.com/api/v1/managed-agents/agents',
    headers=headers,
    json={
...
run = requests.post(
    f"https://suite.sundaypyjamas.com/api/v1/managed-agents/agents/{agent['id']}/runs",
    headers=headers,
    json={'task': 'Summarize the attached Q2 report and list top 3 risks.'},
).json()
```
...
response = requests.post(
    'https://suite.sundaypyjamas.com/api/v1/rag/query',
    headers={'Authorization': f'Bearer {supabase_session_token}'},
    json={'query': 'What were the top risks in Q2?', 'workspace_id': workspace_id},
).json()
...
response = requests.post(
    ...
    headers=headers,
    json={'prompt': 'A minimalist mountain logo, flat design, teal and white'},
).json()

# response['image'] is base64-encoded PNG
```
